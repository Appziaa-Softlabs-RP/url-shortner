...
export default function UrlShortenerForm({
    ...
    const debouncedValidateUrl = useMemo(() => debounce(validateUrl, 300), [validateUrl])
    ...
    const debouncedExtractPageTitle = useMemo(() => debounce(extractPageTitle, 500), [extractPageTitle])
    ...
    useEffect(() => {
        const subscription = form.watch((value, { name, type }) => {
            ...
            const hasChanged = Object.keys(initialValuesRef.current).some((key) => {
                const initialValue = initialValuesRef.current[key as keyof UrlFormValues]
                const currentValue = value[key as keyof UrlFormValues]

                // For title, consider it changed only if it's not empty
                if (key === "title") {
                    return (
                        (currentValue as string).trim() !== (initialValue as string).trim() &&
                            ...
                            <Tooltip>
                                ...
                                <TooltipContent>
                                    <p className="font-normal">
                                        The destination URL can be updated while the short URL remains unchanged.
                                    </p>
                                </TooltipContent>
                            </Tooltip>
                        </TooltipProvider>
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className={urlError ? "text-red-500" : ""}>Destination Link</FormLabel>
                                        <FormControl>
                                            <Input
                                                type="url"
                                        ...
                                        <FormMessage />
                                        {urlError && <FormMessage className="font-light">{urlError}</FormMessage>}
                                    </FormItem>
                                )}
                            />
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormDescription>This will be used as the page title in search results</FormDescription>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            />
